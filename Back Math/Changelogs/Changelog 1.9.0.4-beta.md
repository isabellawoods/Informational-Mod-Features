...
## Hotfix
- Removed the duplicate tooltip on *Back Math*'s shields when *Revaried* is loaded.[^3]
...
- Added wanderer sophie heads.
  ...
  - This is saved to a tag called ![*(string)*](/Revaried/Docs/Tags/string_tag.png) **variant** on the item and block entity.
...
- Chestplates now render on your arm in first person.
  - This doesn't seem to work with dyeable armor like leather or wool for some reason.
- Bakugou armor and insomnia sophie sleepwear no longer give any protection.
...
- Descriptions where my old username (JuneTheWoods) is mentioned have been changed to "**isabellawoods**".

#### Outfits
- File locations can now look at all available mod ids (don't know if resource packs work yet).
- Are now invisible when the entity is invisible, or translucent if the player is in Spectator.
- The fully lit items tag now works with armor outfits.
  - These no longer break on players.
- No longer get a red tint when the wearer takes damage.

### Entities
- The "ender" variant of wanderer sophies now has emissive eyes, for both their face and hoodie.
  - This layer currently renders on top of outfits.
- Archer lucias' inventories are now saved and read using *Revaried*'s stack handling.
...
### Translations
- **[Bra. Portuguese]** Changed the word "viajantes" on wanderer sophie spawn eggs to "andarilhas".
- Moved the translation for the pink gum frying pan description to `tooltip.backmath.pink_gum_frying_pan`.

## Technical
### Additions
- Added [**item behaviors**](/Melony%20Studios%20Wiki/Modded%20Docs/Item%20Behaviors.md).
  ...
  - Behaviors can also define more properties than just functionality:
    - If an item has an enchantment glint (which can be controlled by the ![*(boolean)*](/Revaried/Docs/Tags/boolean_tag.png) **enchantment_glint_override** tag);
  ...
  - Most effect types have a tooltip to show what it does, like apply effects, ignite the entity, give an item, etc.
    - This is shown like *Revaried*'s effect tooltip, except that attributes are grouped together with the effects themselves.
...
- Added *Revaried*'s tag tooltip for the development environment.

## Changes
- Renamed the class *Equippable* to *Swappable* and the method `swapWithEquipmentSlot` to `equipOrSwapItem`, to match *Revaried*.
- When defining the tags `milked_sword_item` and `use_remainder` in the shortened form, the extended form is now written using *Revaried*'s stack handling.
...
- The ![*(string)*](/Revaried/Docs/Tags/string_tag.png) **SophieType** tag from friend sophies (alpha wanderer sophies) is now converted to the new ![*(string)*](/Revaried/Docs/Tags/string_tag.png) **variant** tag.
