![Back Math 9.0.8](Assets/1.9.0.8-beta.png)
<h1 style="text-align: center;">- Back Math - 9.0.8 -</h1>

> **Written On:** 26-07-25 - **Last Updated:** 03-01-26

**9.0.8** is the eighth version for *Back Math* 9.0, released on June 20, 2025.[^1][^2][^3]
...
- Crystallizer recipes are now data-driven.
  ...
  - JEI support is ***very much work in progress***.
...
## Changes
### Blocks
- The inside of crates is now hollow. This means entities can stand inside it like composters.
- Crates no longer clip into the entity's back.
...
- Outfit definitions can now be applied to or changed on any item.
  - This uses the same format as the ![*(string)*](/Revaried/Docs/Tags/string_tag.png) **outfit** tag on entities.
...
- Outfits now render the proper texture on entities.
  - Previously, it would try caching the textures, so chest textures would only render slim/wide depending on which one was cached first.
...
- Tamed queen lucy pets now have a poncho.
  - Ponchos are supposed to be the queen lucy pet's version of a collar.
  - It can be dyed using any vanilla dyes, but the ![*(integer)*](/Revaried/Docs/Tags/integer_tag.png) **poncho_color** tag supports any RGB color.
  - Set to light blue by default, or pure black (`0`) when loading a world from before this change.
- Slightly updated the textures of the "maid" and "worker" wanderer sophie variants.

#### Breasts
- Physics now work properly on mobs that have breasts.
- The breathing animation is now faster when below 4 health.
- Fixed various bugs related to breast rendering:
  - Updated breast outfit rendering, so full-body outfits now render properly.
  - Breathing animation now plays properly (was broken due to wrong chest item check).
...
## Technical
### Additions
- Added a ![*(boolean)*](/Revaried/Docs/Tags/boolean_tag.png) **hides_skin_layers** tag to outfit slot definitions.
...
- Added an update checker to *Back Math*.
  - Currently, it only goes back to [9.0.1](Changelog%201.9.0.1-beta.md), and descriptions were not yet added.
...
- Added Javadoc to various classes and methods in *Back Math*.
...
| Old Name           | New Name                       |
| ------------------ | ------------------------------ |
| *BMColorManager*   | Merged with *BMEventBusEvents* |
...
[^3]: ["Fixed some textures"](https://github.com/isabellawoods/Back-Math/commit/34d4ade16e65c96488a5afce01e7baffde655e0e) (Commit `34d4ade`) – GitHub, November 17, 2025.