...
- ![*(compound)*](/Revaried/Docs/Tags/compound_tag.png) The root object.
  - ![*(string)*](/Revaried/Docs/Tags/string_tag.png) **asset_id**: The resource location of this outfit definition. Should match the filename and namespace of this outfit.
  - ![*(compound)*](/Revaried/Docs/Tags/compound_tag.png) **head**: *(optional)* The head outfit slot. Used when getting a texture for the entity's head.
    ...
    - ![*(int)*](/Revaried/Docs/Tags/integer_tag.png) **color**: *(optional)* An integer defining a set color for this outfit slot. When defined, this field overrides the item's default color and the `display.color` tag.
  - ![*(compound)*](/Revaried/Docs/Tags/compound_tag.png) **chest**, **legs** and **feet**: *(all optional)* Same format as ![*(compound)*](/Revaried/Docs/Tags/compound_tag.png) **head**.
