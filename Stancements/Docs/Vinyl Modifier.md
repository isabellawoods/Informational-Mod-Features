# Vinyl Modifier
> [!NOTE]
> **Written On**: 17-08-26 (5.0.0-beta.1 - 1.21.1) - **Last Updated**: 30-08-26 (5.0.0-beta.2 - 1.21.1 | 0.4.4 - 26.1.2)
...
| `enderscape:music_disc_decay`<sup>*[only in RMS]*</sup> | Ejects the disc after 10-15 seconds.                                                                                                                                                                                                                                              | Something is interfering with the recorder...   | Start      | Yes              |
| `pipeline/finish_recording`                             | Converts the vinyl disc into a recorded disc (based on its `recordable_transform` component), and applies the disc style from the [*recorded disc styles*](Recorded%20Disc%20Style.md) registry. If that's not available, it falls back to randomizing the label style and color. | Finished recording!                             | Finish     | Yes              |
...
- ![*(compound)*](/Revaried/Docs/Tags/compound_tag.png): The root object.
  ...
  - ![*(list)*](/Revaried/Docs/Tags/list_tag.png) **strategies**: When this modifier runs during the recording process. Can be a combination of any of these, but cannot be empty:
    - `before`;
    - `after`;
    - `eject`.
...
- ![*(compound)*](/Revaried/Docs/Tags/compound_tag.png) **effect**
  ...
  - ![*(list)*](/Revaried/Docs/Tags/list_tag.png) **block_particles**: *(26.1.2 only, optional)* Particles for each air block in the radius of the explosion.
    - ![*(compound)*](/Revaried/Docs/Tags/compound_tag.png): A block particle option.
      - ![*(int)*](/Revaried/Docs/Tags/integer_tag.png) **weight**: The weight for this option to be chosen.
      - ![*(compound)*](/Revaried/Docs/Tags/compound_tag.png) **particle**: The particle to spawn.
        - ![*(string)*](/Revaried/Docs/Tags/string_tag.png) **type**: The ID of the particle type. See [Particles (Java Edition)](https://minecraft.wiki/w/Particles_(Java_Edition)) for valid options.
        - **Additional field depending on ![*(string)*](/Revaried/Docs/Tags/string_tag.png) type, see [Particle format § Configurations of particle types](https://minecraft.wiki/w/Particle_format#Configurations_of_particle_types).**
      - ![*(float)*](/Revaried/Docs/Tags/float_tag.png) **scaling**: Scaling of the distance from the center of the explosion to the block position.
      - ![*(float)*](/Revaried/Docs/Tags/float_tag.png) **speed**: Multiplier of the speed of the particle.
...
- ![*(compound)*](/Revaried/Docs/Tags/compound_tag.png) **effect**
  ...
  - ![*(float or compound)*](/Revaried/Docs/Tags/float_tag.png)![](/Revaried/Docs/Tags/compound_tag.png) **radius**: An *float provider* — The radius of the disk.
  - ![*(float or compound)*](/Revaried/Docs/Tags/float_tag.png)![](/Revaried/Docs/Tags/compound_tag.png) **height**: An *float provider* — The height of the disk.
...
Replaces the recordable disc with another item, with the option to keep every data component from the old item.

- ![*(compound)*](/Revaried/Docs/Tags/compound_tag.png) **effect**
  - ![*(compound)*](/Revaried/Docs/Tags/compound_tag.png) **replacement**: An item stack without the ![*(int)*](/Revaried/Docs/Tags/integer_tag.png) **count** field.
  - ![*(boolean)*](/Revaried/Docs/Tags/boolean_tag.png) **keep_components**: *(optional, defaults to `true`)* A boolean — Whether all components from the old item should get transferred to the replacement item.
...
| [5.0.0-beta.2](/Stancements/Changelogs/Changelog%205.0.0-beta.2.md)               | <li>Added the `eject` modification strategy, which runs whenever the recordable disc gets ejected from the music recorder.</li> <li>**[`replace_disk`]** The ![*(int)*](/Revaried/Docs/Tags/integer_tag.png) **radius** and ![*(int)*](/Revaried/Docs/Tags/integer_tag.png) **height** fields are now ![*(float)*](/Revaried/Docs/Tags/float_tag.png) floats.</li> <li>**[`replace_recordable_disc`]** Added an optional ![*(boolean)*](/Revaried/Docs/Tags/boolean_tag.png) **keep_components** field.</li> <li>**[`run_function`]** Slightly updated the error message again.</li>                                                                                                                                                                                            |
| [5.0.0-beta.2](/Stancements/Changelogs/Changelog%205.0.0-beta.2.md) **under RMS** | Added the `enderscape:music_disc_decay` modifier, with the message "*Something is interfering with the recorder...*".                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
