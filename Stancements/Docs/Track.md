# Track
> [!NOTE]
> **Last Updated**: 30-08-26 (5.0.0-beta.2 - 1.21.1)
...
### Codec
If ![*(boolean)*](/Revaried/Docs/Tags/boolean_tag.png) **resolved** is `false`, the track will be stored as an identifier (using `Track.CODEC`). If it's `true` it will use the **direct codec** instead.
...
| [5.0.0-beta.2](/Stancements/Changelogs/Changelog%205.0.0-beta.2.md) | <li>The `music_recorder` block entity's ![*(string)*](/Revaried/Docs/Tags/string_tag.png) **music_id** field has been removed in favor of a new ![*(string or compound)*](/Revaried/Docs/Tags/string_tag.png)![](/Revaried/Docs/Tags/compound_tag.png) **track** field.</li> <li>The `track_storage` component now uses the proper *list codec* instead of calling `Track.CODEC.listOf()`.</li>                                                                                                                           |
