...
- Added the music recorder.
  ...
  - Hoppers can insert and extract discs from this block. Hoppers cannot start recordings, but can take recorded discs out.
...
- Added vinyl discs.
  ...
  - Crafted with 2 coal/charcoal, 1 light gray/gray dye, and 1 honeycomb in a 2x2.
  - Can stack up to 16.
...
- Added the `music_recorder` block entity, with the following fields:
  ...
  - ![*(string)*](/Revaried/Docs/Tags/string_tag.png) **music_id**: A resource location of the song being recorded. Usually, it's the path to the `.ogg` file.
  ...
  - ![*(int array)*](/Revaried/Docs/Tags/integer_array_tag.png) **recorder_player**: *(optional)* The player recording this song. Used to send action bar messages saying the recording has finished.
